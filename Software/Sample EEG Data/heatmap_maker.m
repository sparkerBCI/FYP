...
number_of_vectors = floor(length(o.data(:, 5)) / (epoch_seconds * o.sampFreq));

for i = 1:number_of_vectors
    split_data(i, :) = o.data((1+(i-1)*(epoch_seconds * o.sampFreq)):((epoch_seconds * o.sampFreq)+(i-1)*(epoch_seconds * o.sampFreq)), 5);
end


for observation = 1:number_of_vectors
...
figure();
subplot(2, 1, 1);
imagesc(transpose(psd(1:100, 1:30)));
ylabel('Hz');
xlabel('Observation');
subplot(2, 1, 2);
